...
| `hb_state_task_*` | echte `rinnsal.tasks.client.TaskClient` — seit der TASKPLAN-Extraktion (2026-07-11) eine Fassade auf `taskplan.client.TaskClient` (Tabellenname `rinnsal_tasks` blieb, Default-DB `~/.taskplan/taskplan.db`; Auflösung `[state].task_db_path` > `$TASKPLAN_DB` > `$SCANNER_TASKS_DB` (Legacy) > Default). Bis 0.1.0-alpha.21 zeigte der Default noch auf das entfernte `~/.rinnsal/scanner_tasks.db` (T-20260814-01) | **Umgesetzt** (nur die Task-CRUD-Tools; `hb_state_mem_*`/`hb_state_dispatch` bleiben unverändert bundled — das war nicht Teil des Auftrags). Status-Vokabular übersetzt (`in_progress` ↔ Rinnsals `active`); `cancelled` ist über dieses Tool nicht erreichbar (dokumentierte Lücke, „nur kompatible Operationen"). Agent-Filter beim Update wird von Rinnsal nicht erzwungen (Rinnsal kennt keine Agent-Scope-Guard). Live-Roundtrip verifiziert (MCP-create → echter `TaskClient.get` und umgekehrt, Statusübersetzung geprüft). |
| `hb_mem_*` | echte `usmc.USMCClient` (Facts-Store, `~/.usmc/usmc_memory.db`) | **Umgesetzt (2026-07-23), store/query/context.** USMCs Modell weicht ab (typisierte key/value-Facts, keine Freitext-Suche), daher Modell-Reconciliation im Seam: die homebase-Kategorie bleibt im Fact-Key (`<category>/<uuid>`) erhalten, `hb_mem_query` filtert USMC-Facts client-seitig (`mode: client_filter`), und per-Call-`agent_id`-Provenance bleibt über je einen USMC-Client pro Aufruf erhalten. `hb_mem_merge`/`hb_mem_consolidate` bleiben bundled-only Bulk-Hygiene und melden unter canonical `not_supported` (Folgearbeit, TODO #72). Fehlendes/kaputtes USMC degradiert auf bundled ohne Start-Bruch. Fixture-Roundtrip + Real-Import + Live-Roundtrip gegen den echten USMC-Store verifiziert. Bekannte Grenzen: canonical `hb_mem_store` liefert `key` statt der bundled-`id`; USMCs `get_facts` hat keinen Limit-Parameter → query/context lesen den vollen Facts-Bestand pro Aufruf (Truncation post-fetch); nativer USMC-`domain`-Bestand erscheint in homebase-Queries als Kategorie `fact`. (Die frühere Idee „Gardener als mem-Backend" wurde verworfen: kein `confidence`-Feld, kein natives Merge/Decay.) |
